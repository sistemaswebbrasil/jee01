...
public class CustomPrimeExceptionHandler extends ExceptionHandlerWrapper {
	...
	private static final Logger LOGGER = Logger.getLogger(CustomPrimeExceptionHandler.class.getName());
	...
	protected void handleAjaxException(FacesContext context, Throwable rootCause, ExceptionInfo info)
		...
		// handle custom update / onexception callback
			...
			if (!LangUtils.isValueBlank(handlerComponent.getUpdate())) {
				List<UIComponent> updates = SearchExpressionFacade.resolveComponents(context, handlerComponent,
						handlerComponent.getUpdate());
			...
			if (!LangUtils.isValueBlank(handlerComponent.getOnexception())) {
				...
				writer.write(
						"};hf.call(this,\"" + info.getType() + "\",\"" + EscapeUtils.forJavaScript(info.getMessage())
	...
	protected Throwable buildView(FacesContext context, Throwable throwable, Throwable rootCause) throws IOException {
		if (context.getViewRoot() == null) {
			...
			// if UIViewRoot == null -> 'IllegalArgumentException' is throwed instead of
	...
	protected void handleRedirect(FacesContext context, Throwable rootCause, ExceptionInfo info,
			boolean responseResetted) throws IOException {
		ExternalContext externalContext = context.getExternalContext();
		externalContext.getSessionMap().put(ExceptionInfo.ATTRIBUTE_NAME, info);

		Map<String, String> errorPages = PrimeApplicationContext.getCurrentInstance(context).getConfig()
				.getErrorPages();
		String errorPage = evaluateErrorPage(errorPages, rootCause);

		String url = externalContext.getRequestContextPath() + errorPage;

		// workaround for mojarra -> mojarra doesn't reset
		// PartialResponseWriter#inChanges if we call externalContext#resetResponse
